...
long currentFloor;
String otp = (String)random(0,10)+(String)random(0,10)
    + (String)random(0,10)+(String)random(0,10);
...
void task_HTTPServer() {
  ...
  if (client0){  
    ...
    if(context=="Screen"){
      //Request should contain "yes" or "no" for the light sensor subscription
      //Request should contain "true" or "false" for el App subscription
      if(request.indexOf("yes")!=-1){
        command="detect";
        
      }else if(request.indexOf("no") != -1){
        command="nodetect";
      } else if(request.indexOf("true") != -1) {
        command="correct_password";
      } else if(request.indexOf("false") != -1) {
        command="incorrect_password";
      }
...
void init_lightsensor(){
  String initialDescription = "Name=Lightsensor;Location=Home";
  String initialData = "no";
  originator = "Cae-Lightsensor";
  registerModule("Lightsensor", false, initialDescription, initialData);
...
void init_Screen() {
  String initialDescription = "Name=Screen;Location=Home";
  String initialData = "Nothing yet";
  originator = "Cae-Screen";
  registerModule("Screen", true, initialDescription, initialData);
}
int present = 0;
void task_lightsensor() {
  sensorValue = analogRead(sensorPin);
  if(sensorValue > 80 && present == 0){
      present = 1;
      originator = "Cae-Lightsensor";
      createCI("Lightsensor", DATA_CNT_NAME, "yes");  
  } else if (sensorValue < 80 && present == 1) {
    present = 0;
    originator = "Cae-Lightsensor";
    createCI("Lightsensor", DATA_CNT_NAME, "no"); 
  }
  originator = "Cae-Lightsensor";
}
...
void task_Screen(String cmd) {
  originator = "Cae-Pantalla";
  createCI("Screen", DATA_CNT_NAME, cmd);  
}
void generateOTP(){
  otp = (String)random(0,10)+(String)random(0,10)
    + (String)random(0,10)+(String)random(0,10);
}
void command_Screen(String cmd) {
  Serial.println(cmd);
  lcd.print("uy");
  if (cmd == "detect") {
    lcd.clear();
  ...
  } else if(cmd == "correct_password") {
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print("Password correct");
    lcd.setCursor(0,1);
    lcd.print("Elevator coming");
  } else if(cmd=="incorrect_password") {
    lcd.clear();
    lcd.setCursor(0,0);
    lcd.print(otp);
  }
...
void command_Lightsensor(String cmd) {
...
void setup() {
  ...
  //set SUBs
  originator = "Cae-Screen";
  subscription("Screen", "Lightsensor");
...
void loop() {
  ...
  while(millis()-currentMillis < interval) {
    ...
    if (context != "") {
      if(context=="Screen"){
        command_Screen(command);
        if(command == "detect"){
            task_Screen(otp);
        }
      }else if(context=="Motor"){
